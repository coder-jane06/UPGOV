...
const SECTOR_OFFICER_MAP = OFFICER_BY_SECTOR;
...
function inferCategoryFromText(input) {
  ...
  if (/(pothole|road|waterlog|speed breaker|signage)/.test(text)) {
    return { category: 'Roads & Potholes', subcategory: 'Pothole' };
  }
  if (/(water|pipe|supply|leak|pressure)/.test(text)) {
    return { category: 'Water Supply', subcategory: 'No Water' };
  }
  if (/(garbage|sewer|drain|sanitation|waste|mosquito)/.test(text)) {
    return { category: 'Sanitation', subcategory: 'Garbage Not Collected' };
  }
  if (/(street ?light|lighting|lamp|dark)/.test(text)) {
    return { category: 'Public Lighting', subcategory: 'Street Light Not Working' };
  }
  if (/(electric|power|voltage|transformer)/.test(text)) {
    return { category: 'Electricity', subcategory: 'Power Outage' };
  }
...
async function buildMediaRecord(file, source = 'citizen-upload') {
...
export default function CitizenPortal({ page, onNavigate, onLogout, onPublicDash, onBack }) {
  ...
  const [selectedTicket, setSelectedTicket] = useState(tickets[0]?.ticketId || '');
